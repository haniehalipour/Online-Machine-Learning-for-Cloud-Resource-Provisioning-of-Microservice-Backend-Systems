...
print('LAMBDA: ML MULTICLASS START')
...
TRAINING_DATA_S3_URL = "s3://"+ db('get','{"field_name": "bucket"}') + "/" +db('get','{"field_name": "multiclass_dataset_dir"}')

def publish_to_sns(message, arn, delay):
    time.sleep(delay) #sleep for "delay" second
    if arn == '':
        #use default arn (Controler SNS)
        arn = str(arns[0]["MultiClass_model"])
...
def lambda_handler(event, context):
    ...
    if curr_state == 'state_1':
        ...
        schema_fn = db('get','{"field_name": "multiclass_schema"}')
        recipe_fn = "recipe.json"
        name = "MULTICLASS SAMPLE"
        ...
        db('insert','{"field_name": "multiclass_model_id", "field_value": "'+model_id+'"}')
        #we dont need Evaluation ID for long term, then we will delete it at end
        db('insert','{"field_name": "multiclass_eval_id", "field_value": "'+eval_id+'"}')
        
        #send log message to controller
        new_msg = {"state": "log", "message": "MultiClass_model State_1: Start Creating Model with ID: "+str(model_id)}
        publish_to_sns(new_msg, arns[0]['Controller'], 0)
        print("MultiClass (Model_ID: %s) is Creating" % model_id)
        
        new_msg = {"state": "state_2"}
        publish_to_sns(new_msg, '', 0)
    
    if curr_state == 'state_2':
        model_id = db('get','{"field_name": "multiclass_model_id"}')
        eval_id = db('get','{"field_name": "multiclass_eval_id"}')
        ml = boto3.client('machinelearning') 
        
        #send log message to controller
        new_msg = {"state": "log", "message": "MultiClass_model State_2: Wating for Models to be ready..."}
        publish_to_sns(new_msg, arns[0]['Controller'], 0)
    ...
    if curr_state == 'state_3':
        ...
        s3_ml_output = 's3://'+db('get','{"field_name": "bucket"}')+'/'+db('get','{"field_name": "multiclass_ml_output"}')
        if model_status == 'COMPLETED' and eval_status == 'COMPLETED':
            ...
            db('delete','{"field_name": "multiclass_eval_id"}')
            ...
            if build_model_prediction:
                #send log message to controller
                new_msg = {"state": "log", "message": "MultiClass_model State_3: Start Creating Prediction..."}
                publish_to_sns(new_msg, arns[0]['Controller'], 0)
                
                batch_csv_schema = db('get','{"field_name": "multiclass_batch_csv_schema"}')
                use_model(model_id, batch_csv_schema, s3_ml_output, TRAINING_DATA_S3_URL)
                
        else:
            #send log message to controller
            new_msg = {"state": "error", "message": "MultiClass_model State_3: Creation models Failed..!\n\nModel Status: "+model_status+"\nEvaluation Status: "+eval_status}
            publish_to_sns(new_msg, arns[0]['Controller'], 0)
            
            print("Somethins is wrong, model_status is: " + model_status + " eval_status is: " + eval_status)
        
    if curr_state == 'state_4':
        print ('MultiClass_model State_4: Start Creating Endpoint URL...')
        #send log message to controller
        new_msg = {"state": "log", "message": "MultiClass_model State_4: Start Creating Endpoint URL..."}
        publish_to_sns(new_msg, arns[0]['Controller'], 0)
...
def create_endpoint_url(ml_model_id, context):
    ...
    if endpoint_status == 'FAILED':
        print ("MultiClass_model State_4: Creating Status <<FAILED>>..!")
        #send log message to controller
        new_msg = {"state": "error", "message": "MultiClass_model State_4: Creating Endpoint Status <<FAILED>>..!"}
        publish_to_sns(new_msg, arns[0]['Controller'], 0)
        return False
    elif endpoint_status in ['NONE', 'UPDATING']:
        print ("MultiClass_model State_4: Waiting for Endpoint to be ready...")
        #send log message to controller
        new_msg = {"state": "log", "message": "MultiClass_model State_4: Waiting for Endpoint to be ready..."}
        publish_to_sns(new_msg, arns[0]['Controller'], 0)
        ...
        print ("MultiClass_model State_4: Endpoint is ready and URL is saved into the Database...")
        created_endpoint = response['RealtimeEndpointInfo']['EndpointUrl']
        db('insert','{"field_name": "multiclass_endpoint_url", "field_value": "'+created_endpoint+'"}')
        #send log message to controller
        new_msg = {"state": "log", "message": "MultiClass_model State_4: Endpoint is ready and URL is saved into the Database..."}
        publish_to_sns(new_msg, arns[0]['Controller'], 0)
        #start Multiclass Cross Validating
